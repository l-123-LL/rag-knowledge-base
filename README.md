...
| 版本与生效时间 | `/ingest` 可带 `doc_key`/`version`/`effective_from`/`effective_to`；同一 `doc_key` 只保留最高版本，`/ask` 的 `as_of` 可按指定日期检索当时生效的政策 |
...
| 相关性阈值 | `RAG_MIN_SCORE` 用原始余弦相似度判断"资料不足"，避免硬答（当前语料标定值 0.37） |
| 成本 | 实测单次 RAG 问答约 **1403 tokens / $0.00025**（1000 次约 $0.25）；FAQ、规则、本地工具命中完全不调模型 |
| 转人工闭环 | 投诉 / 明确要求 / 资料不足 / 工具连续失败 → 建单 + 带工单号话术 |
| 副作用审批 | 高风险写操作（退款）默认只出 dry-run 预览并生成审批单，管理员批准后才执行；审批幂等、按租户隔离 |
| 工具组合 | 一次任务可先查订单再补物流轨迹（"订单 X 现在什么状态，物流到哪了"），两个工具的结果合成一条答案；步数上限仍然生效 |
...
- 阈值 0.37 是在当前 5 篇公开语料上校准的（域内最低 0.377 vs 域外最高 0.360，间隙仅 0.017），换真实资料必须用 `backend/evaluation/calibrate_threshold.py` 重新采样。
- 索引里是**公开替代语料**（`backend/corpus/`，Apache-2.0 / MIT / 法规文本），不是企业自有资料。
