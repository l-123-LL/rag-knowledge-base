...
### 2026-09-20 这次会话做完的事（明天从这里往后接）

1. **检索 bug 修复**：分块 id 撞车导致分数错位（正确答案被判"资料不足"），三层修复 + 回归测试。
2. **真实语料入库**：5 篇公开许可资料 / 74 分块，新增幂等导入脚本；来源面板改为从真实索引反推。
3. **阈值重标**：域内最低 0.377 / 域外最高 0.360 → `RAG_MIN_SCORE=0.37`（0.42 在新语料上会误伤 2 条）。
4. **真实语料检索评测**：doc_hit@1 0.95 / evidence_hit@1 0.90 / evidence_mrr 0.925。
5. **成本实测**：单次 RAG 问答 1403 tokens / **$0.000247**，1000 次约 $0.247；1928 次调用里 1910 次零模型成本。
6. **rerank 实测为负收益**（doc_hit@1 0.95→0.85，6.5 s/次）→ 默认关闭，代码与 A/B 命令保留。
7. **两条质量修复**：FAQ 例外语保护（"定制商品"不再被通用答案吃掉）、模型级拒答也补上转人工出口。
8. **多轮追问**：短问句/指代词开头的追问会把上一轮问题并进检索词。
9. **前端专业化改版**：设计系统收口、Markdown 答案渲染、骨架屏、引用列表、侧栏与审批面板重排；README 四张截图重拍。
10. **发布到 GitHub**：<https://github.com/l-123-LL/rag-knowledge-base>，139 提交全历史，CI 三次全绿。
11. **企业格式导入**：新增 Excel / Word / CSV 解析（实测 xlsx/docx/csv 三种都能被正确检索）+ `scripts/ingest_folder.py`。
12. **文档**：新增 `docs/RAG_DESIGN.md`、`docs/INGEST-ENTERPRISE-DATA.md`、`docs/PUBLISH-TO-GITHUB.md`。

**明天优先级最高的三件**（都需要用户提供输入，不是写代码）：

1. 企业自有资料放进 `backend/data/incoming/` → 跑 `scripts/ingest_folder.py` → 重标阈值 → 重跑评测。
2. 录演示视频（`docs/VIDEO-SCRIPT.md`）。
3. 简历贴仓库链接 + 按 `INTERVIEW.md` 练口径。
...
企业智能客服 RAG 问答系统已打通真实闭环：资料导入（PDF/Word/Excel/CSV/HTML/MD）→ 清洗切分 → BGE 向量 + FAISS + BM25 混合检索 → DeepSeek 生成（支持流式）→ 引用展示 → 拒答/转人工/工单 → 执行轨迹与评测。

索引里是 **5 篇许可清晰的公开语料 / 74 个分块**，拒答阈值按实测标定为 **0.37**；代码已发布到 **<https://github.com/l-123-LL/rag-knowledge-base>**（139 提交、CI 两个 job 全绿）。

当前验证状态：后端 **167** 个测试通过 + `ruff` 全绿，前端 **31** 个测试通过 + typecheck/build 通过，Docker 一键起可用，演示库干净。

按功能口径完成度约 **98%**。剩下的只有三类：**① 接入企业自有资料并重标阈值 ② 录演示视频 ③ 简历/面试口径**——都需要用户提供输入，代码主干不需要再改。
