...
| 提交数 | 109 个提交（2026-09-19：检索分块 id 撞车修复 + 5 篇公开语料入库） |
...
结果：**137 passed**（2026-09-19 本轮新增 9 条：重复 id 下分数不错位、分块 id 跨资料唯一、重复导入跳过重复 id、重复导入幂等、FAISS 分数与点积一致、records.json 顺序按内部 id、阈值标定三个纯逻辑用例；覆盖切分、检索、向量库、生成、管线、意图、工具、工作流、轨迹、审批、FAQ、会话、工单、观测、备份、评估等）。
...
外推到真实语料（5 条 600 字 chunk ≈ 2,800 输入 + 250 输出 token）：单次约 **$0.00057（¥0.004）**，即每 1000 次问答约 **$0.57 / ¥4**。注意这条是外推，不是实测；真实数字要等导入企业资料后重测。
