...
### Agent 任务评测（100 条，阶段 3 已完成）

一条命令：`cd backend && ..\.venv\Scripts\python.exe -m evaluation.agent_eval --tag full --use-real-embedder`（15 秒跑完，默认不调用外部模型，**零 API 花费**；评测集构成 = 50 条新增 Agent 任务 + 50 条知识问答）。
...
- 评测分档：`--tag smoke|core|full`（25 / 60 / 100 条）与 `--limit`，日常只跑冒烟，里程碑跑全量并留档。
