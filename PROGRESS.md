...
对比：示例语料时代每次只要 230 tokens / $0.000058，真实长文档的 prompt 大了 6 倍——**"上下文越长越贵"这件事有数字支撑，不是感觉**。同时注意 `/metrics` 里另有一条更重要的数字：1928 次调用中有 1610 次走的是 FAQ / 规则 / 本地工具，**完全没有调模型、成本为 0**，这也是"FAQ 优先"在成本上的意义。
