...
- **应用层故障注入已完成**（2026-09-19，原生部署）：① 故障前请求 200 / 146 ms → ② 杀掉后端进程 → 客户端 **ConnectError，2126 ms 内快速失败、没有挂死** → ③ 重启后端 → 5 秒内健康检查通过，请求恢复 200 / 1167 ms。说明依赖挂了会快速失败而不是拖死调用方，重启后自动可用。
- **容器级故障注入仍未做**：本机 Docker 编排（`docker compose up`）多次挂起，容器一并被清掉，因此「容器中途停止 + 恢复」这条没有实测；排查记录见 `docs/FAILURE-CASES.md` 案例七。
