...
- `backend/tests/`：pytest 测试，覆盖切分、检索、向量库、生成、管线、FAQ、会话、工单、观测、备份、评估、接口。
...
- `web/src/**/*.test.ts(x)`：6 个测试文件，共 18 个用例。
...
**决策：** 检索结果相关性不足时返回「资料不足」并建议转人工。
**原因：** 客服场景编造答案的代价（错误政策、错误承诺）远高于说一句「我帮您转人工」。
