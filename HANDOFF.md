...
**详细进度与实测数字看 `PROGRESS.md`。** 这里只给结论：RAG 闭环 + 可选工具工作流 + 审批 + 评测 + 可观测 + Docker 部署均已实现并实机验证；测试 **167（后端）/ 31（前端）**，`ruff` 全绿；Agent 评测 150 条、任务成功率 99.33%；真实语料检索 evidence_hit@1 0.90。

2026-09-20 的两项进展：代码已推送到 <https://github.com/l-123-LL/rag-knowledge-base>（全量提交历史，GitHub Actions 两个 job 全绿）；资料导入新增 Excel / Word / CSV 解析，配套 `scripts/ingest_folder.py` 批量导入与 `docs/INGEST-ENTERPRISE-DATA.md`。

剩余是需要外部条件的收尾：**接入企业自有资料并重标阈值、录演示视频、简历/面试口径**。
...
17. **rerank 默认关闭（实测负收益）**：`BAAI/bge-reranker-base` 在 74 分块的语料上把 doc_hit@1 从 0.95 拉到 0.85，纯重排延迟 avg 6.5 s / p95 7.5 s（CPU、20 候选），5/20 条问题的 top1 被改坏。所以 `RERANK_MODEL` 留空；代码与 `corpus_eval --compare-rerank` 保留，等语料规模变大、一阶段召回变弱时再开着重测。**不要因为"rerank 通常有收益"就默认打开——这次有反例。**
18. **追问只改检索词、不改生成输入**：多轮里「那丢了怎么赔？」这类问句没有可检索的关键词，所以检索用「上一轮问题 + 本轮问题」（`backend/app/followup.py`，问句 ≤8 字或以指代词开头才触发），而交给模型时仍然只有原始问题和对话历史——避免同一条信息在 prompt 里出现两遍。阈值定在 8 字是因为中文 9 字的「快递丢了怎么赔偿？」已经是完整问句，合并反而会被上一轮话题带偏（这是实测踩出来的）。
